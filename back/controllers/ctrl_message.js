...
exports.cleanMessages = async (req, res, next) => {
    try {
        ...
        `;
        const [result] = await pool.execute(query, [senderId, receiverId, senderId, receiverId]);
        ...
        const deleteQuery = `
            ...
            AND id NOT IN (?);
        `;
        // Utiliser le spread operator pour passer les IDs des messages à garder
        await pool.execute(deleteQuery, [senderId, receiverId, senderId, receiverId, ...messageIdsToKeep]);
